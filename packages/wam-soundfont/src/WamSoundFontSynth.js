 ...
 */
export class WamSoundFontSynth {
	/**
	...
	constructor(parameterInterpolators, samplesPerQuantum, sampleRate) {
		...
		console.log('[WamSoundFontSynth] Initialized with spessasynth_core');
	}
	...
	async loadSoundFont(arrayBuffer) {
		try {
			...
			this.synthesizer.soundBankManager.addSoundBank(soundBank, 'main');

			this.soundFontLoaded = true;

			console.log('[WamSoundFontSynth] SoundFont loaded successfully');
	...
	/**
	 * Generate WAM parameter info
	 */
	static generateWamParameterInfo() {
		return {
			masterVolume: new WamParameterInfo('masterVolume', {
				type: 'float',
				label: 'Master Volume',
				defaultValue: 1.0,
				minValue: 0.0,
			...
			masterPan: new WamParameterInfo('masterPan', {
				type: 'float',
				label: 'Master Pan',
				defaultValue: 0.0,
				minValue: -1.0,
			...
			reverbGain: new WamParameterInfo('reverbGain', {
				type: 'float',
				label: 'Reverb',
				defaultValue: 1.0,
				minValue: 0.0,
			...
			chorusGain: new WamParameterInfo('chorusGain', {
				type: 'float',
				label: 'Chorus',
				defaultValue: 1.0,
				minValue: 0.0,
